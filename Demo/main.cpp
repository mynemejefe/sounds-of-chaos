...
int main(int argc, char* args[])
{
	...
	df::VaoArrays vao((GLuint)MyVAO, GL_TRIANGLE_STRIP, 4, 0u);

	df::ShaderProgramVF fractalShader = "FractalShaderProgram";
	fractalShader << "Shaders/vert.vert"_vert << "Shaders/frag.frag"_frag << df::LinkProgram;
	...
	sam.Run([&](float deltaTime) //delta time in ms
		{
			...
			frameBuff << df::Clear() << fractalShader
			...
			fractalShader << vao;	//Rendering: Ensures that both the vao and program is attached
